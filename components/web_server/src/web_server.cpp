...
esp_err_t web_server::initSoftAP()
{
    ...
    ap_config.ap.authmode = password_.empty() ? WIFI_AUTH_OPEN : WIFI_AUTH_WPA_WPA2_PSK;
...
// ---------------- JSON Utilities ----------------
esp_err_t web_server::applyConfigFromJson(const char *json_data, size_t len) noexcept
{
    if (!json_data || len == 0) return ESP_ERR_INVALID_ARG;
    ...
    item = cJSON_GetObjectItem(root, "chirp_count");
    if (cJSON_IsNumber(item))
        sensor_.set_chirp_integration_count(static_cast<uint8_t>(item->valueint));

    item = cJSON_GetObjectItem(root, "short_filter");
    if (cJSON_IsNumber(item))
