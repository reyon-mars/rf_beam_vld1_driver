...
extern "C" void app_main()
{
    ...
    static uart vld1_uart(UART_NUM_1, 12, 13, 115200, 512); // Setup the uart for the vld1 sensor.
    vld1_uart.init(UART_DATA_8_BITS, UART_PARITY_EVEN, UART_STOP_BITS_1);

    static uart rs485_uart(UART_NUM_2, 17, 16, 115200, 512); // Setup the uart for the rs485 interface.
    static rs485 rs485_slave(rs485_uart, GPIO_NUM_5);
    rs485_slave.init(1, MB_PARAM_INPUT, 3); // Setup the rs485 with the MODBUS function type and register sizes.

    static led main_led(GPIO_NUM_2);
    main_led.init();
    main_led.blink(1, 100);

    static vld1 vld1_sensor(vld1_uart); // Initialize the vld1_sensor with the  vld1 uart communication inteface.
